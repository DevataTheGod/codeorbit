# IDE File System Integration Guide (IDE_ARCHITECTURE.md)

## Overview

The CodeOrbit IDE includes a complete file management system with full access to project files, structure, and localStorage persistence. The Socratic AI mentor can see and reference the entire project while providing guidance.
...
### File System Service (`frontend/src/services/IDEFileSystem.ts`)
...
### File System Context Hook (`frontend/src/hooks/useFileSystem.ts`)
...
### File Operations Panel (`frontend/src/components/ide/FileOperationsPanel.tsx`)
...
Files are stored as JSON in `localStorage['CODEORBIT_IDE_FILES']`:
...
2. Downloads `codeorbit-project-{timestamp}.json`
...
### New Files
- `frontend/src/services/IDEFileSystem.ts` — Core file system service
- `frontend/src/hooks/useFileSystem.ts` — React context hook
- `frontend/src/components/ide/FileOperationsPanel.tsx` — UI for file operations

### Updated Files
- `frontend/src/components/ide/IDEWorkspace.tsx` — Integrated FileSystemProvider
- `frontend/src/components/ide/AIChatPanel.tsx` — Now receives project files
- `supabase/functions/orbit-chat/index.ts` — Enhanced to use project context
